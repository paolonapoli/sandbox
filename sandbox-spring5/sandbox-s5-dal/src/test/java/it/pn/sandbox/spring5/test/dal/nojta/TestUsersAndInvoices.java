/*
 * File TestUsersAndInvoices.java of project sandbox-s5-dal.
 * File created on 22 ott 2017 at PN-HQ.
 */
package it.pn.sandbox.spring5.test.dal.nojta;
...
public class TestUsersAndInvoices {
    @Autowired
    ...
    @Transactional
    // @Commit // does the COMMIT at the end
    public void pupulateUsersAndInvoiceExceptionRollbackOnlyPrimaryTxManager() {
	insertUser();
    ...
    // @Commit // does the COMMIT at the end
    public void pupulateUsersAndInvoiceExceptionTxOracle() {
	insertUser();

	insertInvoice();

	throw new RuntimeException("Volontary exception to test rollback (works on Oracle TX manager)");
    }

    @Test
    @Transactional("mysqlTransactionManager")
    // @Commit // does the COMMIT at the end
    public void pupulateUsersAndInvoiceExceptionTxMysql() {
	insertUser();

	insertInvoice();

	throw new RuntimeException("Volontary exception to test rollback (works on MySQL TX manager)");
    }
